...
export default function SubjectDetailPage() {
  ...
  const status = assignmentForm.watch("status");
  ...
  const handleAssignmentSubmit = async (values: z.infer<typeof assignmentSchema>) => {
    try {
      ...
      if (editingAssignment) {
        ...
        await addAssignment({ ...data, subjectId: id });
  ...
  const renderMaterial = (material: StudyMaterial) => {
    if (material.contentType === 'link') {
      return <a href={material.content} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{material.title}</a>;
    }
    return (
        <Dialog>
              ...
              <Table>
                ...
                <TableBody>
                  {assignments.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center text-muted-foreground">No assignments yet.</TableCell>
                    </TableRow>
                    ...
                    <TableRow key={assignment.id}>
                      ...
                      <TableCell>
                        {assignment.status === 'Completed' && assignment.grade !== null ? `${assignment.grade}%` : 'N/A'}
                      </TableCell>
                            ...
                            {status === 'Completed' && (
                                <FormField control={assignmentForm.control} name="grade" render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl><Input type="number" placeholder="Enter grade (0-100)" {...field} /></FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}/>
                            )}
